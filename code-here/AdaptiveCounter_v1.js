...
/*
 * 공격 성향 스위치. 자동 튜너가 덮어쓰지 않는 값들이라 실제 경기에서
 * "서브/네트 공격/블로킹" 감각만 빠르게 손볼 때는 이 블록을 조정한다.
 */
var ATTACK = {
  NET_ATTACK_RANGE: 68,
  NET_FAST_BONUS: 34,
  BLOCK_OPP_NET_RANGE: 132,
  BLOCK_BALL_DX: 76,
  BLOCK_BALL_DY: 88
};
...
function updateGenericProfile(s, isLeft, incomingLand) {
  if (M.v11ProfileLocked || M.v9ProfileLocked || M.제출본ProfileLocked) return;
  ...
  var 제출본Route = speed === 20 && s.opp.y === 136 && (
...
function chooseShot(contact, s, isLeft) {
  ...
  for (var i = 0; i < SHOTS.length; i++) {
    ...
    if (type.fast && Math.abs(contact.x - NET) <= ATTACK.NET_ATTACK_RANGE) {
      geometry += ATTACK.NET_FAST_BONUS;
    }
...
function shouldPreJump(s, isLeft, anchor, predictedX, group) {
  ...
  // state=2는 상대가 강타를 이미 장전한 확정 신호다. Jayce형 프로필이 잠긴
  // 뒤에는 두 번째 표본을 기다리지 않고 네트 앞에서 먼저 뜬다.
  var immediatePowerPose = M.counterLocked && s.opp.state === 2 &&
    Math.abs(s.opp.x - NET) <= ATTACK.BLOCK_OPP_NET_RANGE &&
    Math.abs(s.ball.x - s.opp.x) <= ATTACK.BLOCK_BALL_DX &&
    Math.abs(s.ball.y - s.opp.y) <= ATTACK.BLOCK_BALL_DY;
  // 그 외에는 반복 코스가 확인된 상대에게만 선제 블로킹하여 깊은 공 수비를 남긴다.
  var armedThreat = immediatePowerPose || (M.oppLandingCount >= 2 && repeatsZone &&
...
function genericPolicy(s, isLeft, minX, maxX, predictedX, group, path, land) {
  ...
  if (s.self.state === 1) {
    if (plan.f <= clamp(GP.POWER_LEAD + TUNE.GENERIC_POWER_LEAD_BIAS, 3, 12)) {
      // 일반 모드도 x를 항상 비영으로 넣으므로 모든 파워 접촉은 속도 20의
      // 강타다. 각도만 안전 시뮬레이터에 맡겨 다양한 상대에게 네트사를 막는다.
      var shot = genericShot(plan, s, isLeft, M.touchCount >= 3);
      M.selectedShot = shot.index;
      x = sign(plan.x - s.self.x) || towardNet;
      // 안전 후보가 계산되지 않아도 서브만큼은 취소하지 않는다. x 비영(20속도)
      // + y=-1 강아치가 네트 자책점보다 훨씬 안전하다.
      if (!shot.ok && isServe) return { x: x, y: -1, hit: 1 };
...
function decideCore(s) {
  ...
  GP = M.v11ProfileLocked ? GP_V11_COUNTER :
    ((M.v9ProfileLocked || M.제출본ProfileLocked) ? GP_V9_COUNTER : GP_BASE);
