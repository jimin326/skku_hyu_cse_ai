...
3. **당일 스킬 대응.** 공개된 규칙 코드를 읽고 발톱 사용을 얹었습니다. 내 게이지가 차고 내 발톱이 비행 중이 아니면 상대가 서 있는 자리에 발톱을 씁니다. 상대 발톱은 피하지 않고 공을 받는 쪽을 택했습니다.
...
| 9월 5일 대회 | 스킬 공개 → 규칙 코드 정독 → 시뮬레이터에 이식 → 발톱 사용 추가 → 게이트와 브라우저 경기 통과 → 제출, 우승 |
...
실제 제출한 파일은 `bot/submitted/사자먹는은행_v1.js` 입니다. v13_1 에서 바뀐 부분과 검증 결과는 `bot/submitted/SUBMISSION.md` 에 정리했습니다.
...
# 제출 게이트 (약 1분)
node --no-warnings tools/dayof/gates.mjs "bot/submitted/사자먹는은행_v1.js" --base Lion_Eating_Bank_v13 --allow-shadow-diff "제출본 손편집"
# 발톱 스킬 켜고 예제 봇과 6경기
node --no-warnings tools/eval_skill_real.mjs ./skills/claw_real.mjs "bot/submitted/사자먹는은행_v1.js" skill-example_v1 3
# 헤드리스 Chrome 에서 실제 경기 (약 2분)
node tools/dayof/harness_dayof.mjs ../engine --bot "submitted/사자먹는은행_v1.js" --opp skill-example_v1.js --score 5
```

눈으로 경기를 보려면 `../engine` 에서 `npm start` 로 게임을 띄우고, 이 저장소의 `bot/submitted/사자먹는은행_v1.js` 를 `../engine/src/code-here/` 에 복사한 뒤 화면의 봇 설정에서 고르면 됩니다. PowerShell 은 `npm.cmd`, `$env:ENGINE_ROOT = '..\engine'` 을 씁니다. 셸별 명령은 `tools/dayof/SHELL_COMMANDS.md` 에 있습니다.

## 저장소 구조

```
bot/                 봇들. submitted/ 에 실제 제출본(당일 스킬 대응 포함), Lion_Eating_Bank_v12~v13 이 대회 전 최종 라인(설계 문서 .md 포함),
...
성균관대 소프트웨어학과 동기 셋이 참가했습니다. 역할은 브랜치와 커밋 기준입니다.

[@jimin326](https://github.com/jimin326) — 최종 봇 설계와 구현: 서브 한 방(썬더 서브) 발견과 시퀀스 제작, 상대 도달 범위를 계산해 확실할 때만 강타하는 랠리 엔진과 수비 위치, 외부 리뷰로 드러난 물리 예측 오류 수정. 프레임 단위 오프라인 시뮬레이터, 출력 동일 검사, 규칙 검사, 제출 게이트, 헤드리스 Chrome 하네스 등 검증 도구 일체. 스킬 공개 전 발톱·게이지 예측 문서와 당일 계획서, 당일 공개된 스킬을 시뮬레이터에 이식해 정책 검증.

[@keunhyung](https://github.com/keunhyung) — 대회 당일 스킬 대응(공개된 발톱 규칙 분석과 적용). 초기 탐색 봇 `Sajamokneun` 시리즈, 물리 기반 수비 상태기 `Lion_Eating_Bank_v6_Defense`, v11 검증 보고서, 강화학습·모방학습 실험(`robust-rl-colab`).

[@kimmykimmim](https://github.com/kimmykimmim) — 상대의 공격 코스를 온라인으로 학습해 수비 위치를 바꾸는 `AdaptiveCounter` v1~v6 시리즈(v5_2는 최종 봇의 검증 상대), 수비 봇에 공격 루트를 얹은 `mixed` v1~v2, v11 최적화판(접촉 횟수·서브 학습 초기화 버그 수정, 계산 경량화).
