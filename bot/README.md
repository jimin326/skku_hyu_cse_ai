...
도구(`tools/`)는 봇을 **이름만 주면 이 폴더에서** 찾는다(`Lion_Eating_Bank_v13` → `bot/Lion_Eating_Bank_v13.js`). 경로를 주면 어디든 된다.

| 파일 | 무엇 |
|---|---|
| `submitted/사자먹는은행_v1.js` | **실제 제출본.** v13_1 위에 당일 공개된 발톱 스킬 대응을 얹은 판. 변경 내용·검증: `submitted/SUBMISSION.md` |
| `Lion_Eating_Bank_v13.js` / `.md` | v12 + 외부 리뷰 F1~F3 물리 예측 수정. 제출본의 shadow_diff 기준 |
...
계보: v1(썬더 서브) → v3(3위상) → v4(밴딧·이탈) → v5_2(상태 인지 도달 모델) → v8~v10(수비 위치·넘기기 시뮬) → v11(썬더 리시브, FLY-3) → v12(클린코드) → v13(물리 수정) → 제출본(발톱).
