 ...
 * 버전 제출본 / 세대 18
 * 적합도 23.19 (내장AI 마진 8.0, 챔피언 마진 7.6)
...
/* ---------- 1. 튜닝 파라미터 ---------- */
var P = {
  HORIZON:        93,   // 몇 프레임 앞까지 공 궤적을 예측할지
  LATENCY:         3,   // 입력이 반영되기까지의 프레임 (3프레임 주기 + 1틱 지연)
  DEADBAND:        12,   // 목표 x와 이만큼 벌어져야 이동
  HOME_FROM_NET:  154,   // 상대 턴일 때 대기 위치
  WALL_MARGIN:    40,   // 코트 끝에서 남길 여유 (플레이어 반폭이 32)

  REACH_X:        24,   // 충돌 32 중 실제로 노릴 여유 (예측 오차 마진)
  REACH_Y:        21,

  /* 타격 지점 선호도 (점수가 높은 계획을 고른다) */
  W_HEIGHT:      2.27,  // 높은 곳에서 칠수록 가산 (y가 작을수록)
  W_NET:         2.03,  // 네트에 가까울수록 가산
  W_EARLY:       0.81,  // 빨리 칠수록 가산

  /* 파워히트 타이밍 */
  POWER_LEAD:      7,   // 접촉 몇 프레임 전부터 hit=1 을 눌러둘지
  JUMP_MAX_DELAY: 22,   // 몇 프레임 뒤 점프까지 계획에 넣을지

  /* 각도 결정 (파워히트 순간의 y 입력) */
  FLAT_MAX_Y:    113,   // 접촉 높이가 이보다 위(y<값)일 때만 y=0 수평강타 허용
  SMASH_MAX_Y:   108,   // 이보다 더 위 + 네트 근처면 y=1 내리꽂기
  SMASH_NET_DIST: 48,
  OPP_NEAR_NET:   147,   // 상대가 네트에 붙어 있으면 내리꽂기 대신 아치로 뒤를 노림

  /* 서브 전용 — 궤적이 결정적이므로 별도 처리 */
  SERVE_MAX_FRAME: 17,  // 랠리 시작 후 이 프레임 안쪽이면 서브 상황
  SERVE_FLAT_MAX_Y:212, // 서브 접촉이 이보다 위(y<값)면 y=0 수평 스파이크
  SERVE_W_HEIGHT:  4.84, // 서브 때는 높은 접촉을 훨씬 강하게 선호
  SERVE_COMMIT:    35,  // 서브는 접촉까지 멀어도 점프 계획을 실행

  /* 타구 결과 시뮬레이션 (v8 신규) */
  NET_MARGIN:     32,   // 네트 통과 시 이만큼 여유가 없으면 그 각도는 버린다
  W_LAND_DIST:   2.49,  // 낙하지점이 상대에게서 멀수록 가산
  W_LAND_TIME:   0.99,  // 빨리 떨어질수록 가산 (반응 시간을 뺏음)
  W_NET_SAFE:    0.19,  // 네트 여유가 클수록 가산

  /* 다이빙 (슬라이드) */
  DIVE_ENABLE:  true,
  DIVE_SPEED:      8,   // 다이빙 수평 속도 (엔진값)
  DIVE_MAX_GAP:  161,   // 걸어서 못 가는 거리가 이 안쪽이면 다이빙으로 시도
  DIVE_MIN_GAP:    15,   // 이보다 가까우면 굳이 다이빙 안 함
